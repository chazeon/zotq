...
- Still transitional:
  - Search SQL still joins `documents` for compatibility until query-path cutover is validated.
  - `documents` is still dual-written for compatibility during migration.
  - Legacy normalized columns in `documents` are still dual-written for compatibility.
  - Final full `items`-first query-path cutover remains pending.
...
1. Complete v2 schema split:
   - Canonical metadata storage is now mirrored into `items` with compatibility reads preserved.
   - Complete query-path cutover from `documents` joins to `items` joins where safe.
